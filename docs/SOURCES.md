# Sources and Downloads

Always use the current instructions from the original project. The tested versions in this repository are historical evidence, not a recommendation to seek old binaries from mirrors.

## Primary projects

- [OptiScaler repository](https://github.com/optiscaler/OptiScaler)
- [OptiScaler wiki](https://github.com/optiscaler/OptiScaler/wiki)
- [ReShade official site and download](https://reshade.me/)
- [RenoDX repository](https://github.com/clshortfuse/renodx)
- [DLSS5-Feeder repository](https://github.com/jlrouzies-fr/DLSS5-Feeder)
- [DLSS5-Feeder releases](https://github.com/jlrouzies-fr/DLSS5-Feeder/releases)
- [RHI installer and releases](https://github.com/RankFTW/RHI/releases)
- [RHI release repository](https://github.com/RankFTW/rhi-repo/releases)
- [LumeniteFX](https://github.com/umar-afzaal/LumeniteFX)
- [dgVoodoo2 repository](https://github.com/dege-diosg/dgVoodoo2)
- [dgVoodoo2 official site](https://dege.freeweb.hu/)
- [Hogwarts Legacy Ultra Plus](https://www.nexusmods.com/hogwartslegacy/mods/660)
- [DOOM: The Dark Ages PC system requirements](https://help.bethesda.net/app/answers/detail/a_id/68551/)
- [NVIDIA Streamline DLSS Frame Generation programming guide](https://github.com/NVIDIA-RTX/Streamline/blob/main/docs/ProgrammingGuideDLSS_G.md)
- [NVIDIA Streamline DLSS Ray Reconstruction programming guide](https://github.com/NVIDIA-RTX/Streamline/blob/main/docs/ProgrammingGuideDLSS_RR.md)

## NVIDIA information

- [NVIDIA App release highlights](https://www.nvidia.com/en-eu/software/nvidia-app/release-highlights/)
- [NVIDIA DLSS 4.5 and Dynamic Multi Frame Generation](https://www.nvidia.com/en-us/geforce/news/dlss-4-5-dynamic-multi-frame-gen-6x-2nd-gen-transformer-super-res/)
- [NVIDIA DLSS overview](https://www.nvidia.com/en-us/geforce/technologies/dlss/)

## Supply-chain rules

- Do not download repacked DLL collections from random file hosts.
- Check release signatures or hashes when the project publishes them.
- Preserve upstream license and attribution files.
- Do not redistribute ReShade or NVIDIA binaries from this repository.
- Treat a modified or `HashMismatch` NVIDIA DLL as untrusted. The DOOM experiment recorded such a file and did not publish it.

## Mod catalogs

Each game catalog links directly to the original mod-author page and records its own review date:

- [DOOM: The Dark Ages catalog](../games/doom-the-dark-ages/MOD-CATALOG.md)
- [Hogwarts Legacy catalog](../games/hogwarts-legacy/MOD-CATALOG.md)
- [Bayonetta catalog](../games/bayonetta/MOD-CATALOG.md)
