...
template<class Fn> void Visit(Config* c, Fn fn)
{
    ...
    fn("PrivateUpscaler", c->DlssNrPrivateUpscaler);
...
{
    Json result = {{"Precision", 0}, {"ResidualFg", false}, {"ResidualFgApproxCamera", false}};
...
inline bool Ensure(Config* c)
{
    ...
    try {
        ...
        if (std::filesystem::exists(Path())) {
            ...
            for (int i = 0; i < 4; ++i) { loaded[i] = file.at("slots").at(i);
                // v0.8 adds these controls; preserve all existing and retired values.
                for (const auto* key : {"PrivateUpscaler"})
                    if (!loaded[i].contains(key)) loaded[i][key] = initial.at(key);
                Validate(c, loaded[i]); }
...
inline bool Save(Config* c)
{
    ...
    try {
        auto updated = slots;
        auto captured = Capture(c);
        for (const auto* key : {"Precision", "ResidualFg", "ResidualFgApproxCamera"})
            if (updated[active].contains(key)) captured[key] = updated[active].at(key);
        updated[active].update(captured); // Preserve retired and unknown fields.
...
inline void RenderSelector(Config* c)
{
    ...
    Visit(c, [&](const std::string& name, auto& option) {
        const auto value = option.value_for_config();
        if (slots[active].at(name) != (value ? Json(*value) : Json(nullptr))) modified = true;
    });
